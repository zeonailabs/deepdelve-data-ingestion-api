...
from rest_api.db.models.survey_insert_request import SurveyInsertRequest, SurveyMetaInsertRequest, \
    SurveySearchInsertRequest, SurveyFeedbackInsertRequest
from rest_api.db.schemas.survey_insert_request import SurvInsReqCreate, SurvMetaInsReqCreate, SurvUpReqCreate, \
    SurvMetaUpReqCreate, SurvSearchInsReqCreate, SurvSearchUpReqCreate, SurvFeedInsReqCreate, SurvFeedUpReqCreate
...
class CRUDSurvMetaInsReq(CRUDBase[SurveyMetaInsertRequest, SurvMetaInsReqCreate, SurvMetaUpReqCreate]):
    ...
    def get_req_id_for_meta(self, db: Session, filters):
        if filters[2]:
            return db.query(self.model.surveyReqId).filter(and_(*filters[0])).filter(or_(*filters[1])).filter(
                not_(*filters[2])).all()
        else:
